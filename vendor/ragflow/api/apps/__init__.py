...
_LOCAL_PROXY_ADDRESSES = {"127.0.0.1", "::1", "::ffff:127.0.0.1"}
_SHOPMIND_TENANT_HEADER = "X-ShopMind-Tenant-ID"


def _local_ops_no_login_enabled():
    return os.getenv("RAGFLOW_ADMIN_NO_LOGIN", "false").strip().lower() in {"1", "true", "yes", "on"}


def _load_local_shopmind_user(auth_types):
    """Load the dedicated ShopMind tenant through the bundled Nginx only."""
    if AUTH_JWT not in auth_types or not _local_ops_no_login_enabled():
        return None
    if (request.remote_addr or "").strip() not in _LOCAL_PROXY_ADDRESSES:
        return None
    configured_tenant_id = os.getenv("RAGFLOW_SHOPMIND_TENANT_ID", "").strip()
    requested_tenant_id = request.headers.get(_SHOPMIND_TENANT_HEADER, "").strip()
    if not configured_tenant_id or requested_tenant_id != configured_tenant_id:
        return None
    try:
        users = UserService.query(id=configured_tenant_id, status=StatusEnum.VALID.value)
    except Exception:
        logging.exception("Failed to load ShopMind tenant user")
        return None
    if users and getattr(users[0], "is_active", False):
        g.auth_type = AUTH_JWT
        g.user = users[0]
        return users[0]
    logging.error("Configured ShopMind tenant user is unavailable")
    return None


def _load_local_ops_user(auth_types):
    """Authenticate only requests proxied by the bundled local Nginx.

    The operations UI is published on host loopback port 8081. Requests from
    that UI reach the API server from Nginx on loopback, while direct calls to
    port 9380 and calls from other containers do not. This keeps API-key/JWT
    authentication intact for the product/API channel.
    """
    if AUTH_JWT not in auth_types or not _local_ops_no_login_enabled():
        return None
    if (request.remote_addr or "").strip() not in _LOCAL_PROXY_ADDRESSES:
    ...
    try:
        ...
        logging.exception("Failed to load local operations administrator")
        return None
    for user in users:
        if getattr(user, "is_active", False):
            ...
            g.user = user
            return user
    logging.error("Local operations no-login is enabled but no active superuser exists")
    return None
...
def _load_user(auth_types=None):
    ...
    local_shopmind_user = _load_local_shopmind_user(auth_types)
    if local_shopmind_user:
        return local_shopmind_user

    local_ops_user = _load_local_ops_user(auth_types)
    if local_ops_user:
        return local_ops_user

...
class LiteLLMBase(ABC):
    ...
    def _clean_conf(self, gen_conf):
        ...
        if self.provider == SupportedLiteLLMProvider.DeepSeek:
            ...
            if self.provider == SupportedLiteLLMProvider.Ollama:
                # Local Ollama models can otherwise keep generating until the
                # full context window is consumed, especially when a small
                # model starts repeating itself. Use conservative deterministic
                # defaults for customer-facing RAG answers while preserving
                # explicit assistant settings.
                gen_conf.setdefault("temperature", float(os.getenv("RAGFLOW_OLLAMA_TEMPERATURE", "0")))
                gen_conf.setdefault("top_p", float(os.getenv("RAGFLOW_OLLAMA_TOP_P", "0.2")))
                raw_limit = gen_conf.get("max_tokens") or os.getenv("RAGFLOW_OLLAMA_MAX_TOKENS", "128")
                try:
                    ollama_max_tokens = max(64, min(int(raw_limit), 2048))
                except (TypeError, ValueError):
                    ollama_max_tokens = 128
                gen_conf["max_tokens"] = ollama_max_tokens

                extra_body = gen_conf.get("extra_body")
                if not isinstance(extra_body, dict):
                    extra_body = {}
                options = extra_body.get("options")
                if not isinstance(options, dict):
                    options = {}
                options.setdefault("repeat_penalty", float(os.getenv("RAGFLOW_OLLAMA_REPEAT_PENALTY", "1.2")))
                options.setdefault("repeat_last_n", int(os.getenv("RAGFLOW_OLLAMA_REPEAT_LAST_N", "128")))
                options.setdefault("num_predict", ollama_max_tokens)
    ...
    def _construct_completion_args(self, history, stream: bool, tools: bool, **kwargs):
        ...
        # Qwen3 and other reasoning-capable Ollama models may place the entire
        # response in Ollama's `thinking` field. RAG answers must return normal
        # user-facing content, so disable thinking by default for Ollama. This
        # remains configurable for non-customer-facing workloads.
        if self.provider == SupportedLiteLLMProvider.Ollama:
            ollama_thinking = os.getenv("RAGFLOW_OLLAMA_THINKING", "false").strip().lower() in {"1", "true", "yes", "on"}
            completion_args.setdefault("think", ollama_thinking)

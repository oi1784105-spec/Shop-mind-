...
def test_ollama_uses_bounded_deterministic_rag_defaults(monkeypatch):
    monkeypatch.delenv("RAGFLOW_OLLAMA_MAX_TOKENS", raising=False)
    cleaned = _make_litellm("ollama_chat/qwen3:8b", SupportedLiteLLMProvider.Ollama)._clean_conf({})

    assert cleaned["temperature"] == 0
    assert cleaned["top_p"] == 0.2
    assert cleaned["max_tokens"] == 128
    assert cleaned["extra_body"]["enable_thinking"] is False
    assert cleaned["extra_body"]["options"] == {
        "repeat_penalty": 1.2,
        "repeat_last_n": 128,
        "num_predict": 128,
    }
...
def test_ollama_preserves_explicit_generation_settings():
    cleaned = _make_litellm("ollama_chat/qwen3:8b", SupportedLiteLLMProvider.Ollama)._clean_conf(
        {
            "temperature": 0.4,
            "top_p": 0.7,
            "max_tokens": 256,
            "extra_body": {"options": {"repeat_penalty": 1.35, "num_predict": 192}},
        }
    )

    assert cleaned["temperature"] == 0.4
    assert cleaned["top_p"] == 0.7
    assert cleaned["max_tokens"] == 256
    assert cleaned["extra_body"]["options"]["repeat_penalty"] == 1.35
    assert cleaned["extra_body"]["options"]["num_predict"] == 192
    assert cleaned["extra_body"]["options"]["repeat_last_n"] == 128

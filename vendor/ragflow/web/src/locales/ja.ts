  ...
  translation: {
    ...
    knowledgeDetails: {
      ...
      similarityThresholdTip:
        'ShopMindは、ユーザークエリとチャンク間の類似度スコアがこのしきい値を下回る場合、そのチャンクを結果から除外します。デフォルトでは、閾値は0.2に設定されています。これは、ハイブリッド類似度スコアが20以上のチャンクのみが取得されることを意味します。',
      ...
      rerankTip: `任意です。空欄の場合、ShopMindは加重キーワード類似度と加重ベクトルコサイン類似度の組み合わせを使用します。リランキングモデルが選択された場合は、加重リランキングスコアが加重ベクトルコサイン類似度に代わります。リランキングモデルを使用すると、システムの応答時間が大幅に増加することにご注意ください。リランキングモデルを使用する場合は、SaaSリランカーを使用してください。ローカルにデプロイされたリランキングモデルを使用する場合は、docker-compose-gpu.ymlでShopMindを起動してください。`,
      topK: 'トップK',
      ...
      retrievalTestingDescription:
        '検索テストを実行し、ShopMindがLLMに必要な内容を正しく取得できるかを確認します。',
      Parse: 'パース',
...
上記が要約する内容です。`,
      ...
      tableColumnModeAutoDescription:
        'すべての列がチャンクテキストに含まれ、メタデータとしても保存されます（ShopMindのデフォルト）。',
    ...
    chat: {
      ...
      variable: '変数',
      variableTip: `ShopMindのチャットアシスタント管理APIと組み合わせて使用することで、変数はより柔軟なシステムプロンプト戦略を開発するのに役立ちます。定義された変数は、LLMのプロンプトの一部として「システムプロンプト」で使用されます。{knowledge}は、指定された知識ベースから取得された部分を表す特別な予約変数であり、「システムプロンプト」ではすべての変数を波括弧{}で囲む必要があります。詳細はhttps://ragflow.io/docs/dev/set_chat_variablesを参照してください。`,
      add: '追加',
      key: 'キー',
      optional: 'オプション',
      operation: '操作',
      ...
      betaError: 'システム設定ページからShopMind APIキーを取得してください。',
      searching: '検索中...',
      parsing: '解析中',
      uploading: 'アップロード中',
    ...
    setting: {
      ...
      embeddingModelTip:
        '新しく作成された各ナレッジベースのデフォルト埋め込みモデルです。ドロップダウンから埋め込みモデルが見つからない場合は、ShopMindスリムエディション（埋め込みモデルを含まない）を使用しているか、https://ragflow.io/docs/dev/supported_models を確認して、モデルプロバイダーがこのモデルをサポートしているかどうかを確認してください。',
    ...
    fileManager: {
      ...
      uploadDescription:
        'ShopMindは、単一またはバッチでのファイルアップロードをサポートします。ローカルにデプロイされた ShopMind の場合: アップロードごとの合計ファイルサイズ制限は 1GB、バッチアップロードの制限は 32 ファイルです。アカウントごとのファイル総数には制限がありません。cloud.ragflow.io の場合: アップロードごとの合計ファイルサイズ制限は 10MB、各ファイルは 10MB を超えず、アカウントごとに最大 128 ファイルまでです。',

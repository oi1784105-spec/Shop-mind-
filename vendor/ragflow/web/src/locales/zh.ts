...
例如：一条带有 1024 维 Embedding 的 1 KB 消息大约使用 9 KB。5 MB 的默认限制大约可容纳 500 条此类消息。`,
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
         '进行检索测试，检查 ShopMind 是否能够为大语言模型（LLM）恢复预期的内容。',
      Parse: '解析',
      dataset: '知识库',
      testing: '检索测试',
      configuration: '配置',
      ...
      parsingStatus: '解析状态',
      parsingStatusTip:
         '文本解析的时间取决于诸多因素。如果开启了 Graph、RAPTOR、自动问题提取、自动关键词提取等功能，时间会更长。如果解析进度条长时间不更新，请联系内部运维人员。',
      processBeginAt: '开始于',
      ...
      rerankTip: `非必选项：若不选择 Rerank 模型，系统将默认采用关键词相似度与向量余弦相似度相结合的混合查询方式；如果设置了 Rerank 模型，则混合查询中的向量相似度部分将被 Rerank 打分替代。请注意：采用 Rerank 模型会非常耗时。如需选用 Rerank 模型，建议使用 SaaS 的 Rerank 模型服务；如果你倾向使用本地部署的 Rerank 模型，请务必确保你使用 docker-compose-gpu.yml 启动 ShopMind。`,
      topK: 'Top-K',
      ...
      autoQuestions: '自动问题提取',
       autoQuestionsTip: `利用在“配置”中指定的索引模型对知识库的每个文本块提取 N 个问题以提高其排名得分。请注意，开启后将消耗额外的 Token。您可以在块列表中查看、编辑结果。如果自动问题提取发生错误，不会妨碍整个分块过程，只会将空结果添加到原始文本块。`,
      autoTags: '自动标签提取',
...
<p>编译后的知识单元以分块形式合并进分块流输出，适合在分块文档之上构建可检索的知识层。</p>`,
    ...
    <p>
      ...
      我们不是将简历分块，而是将简历解析为结构化数据。 作为 HR，你可以扔掉所有的简历，
       您只需与<i>'ShopMind'</i>交谈即可列出所有符合资格的候选人。
      </p>
        `,
...
示例：general/v2/`,
      ...
      ttsModel: 'TTS',
      ttsModelTip:
         '默认的tts模型会被用于在对话过程中请求语音生成时使用。如未显示可选模型，请检查本地模型提供商配置。',
      embeddingModel: 'Embedding',
      embeddingModelTip:
         '所有新创建的知识库使用的默认 Embedding 模型。如未显示可选模型，请检查本地模型提供商配置。',
      img2txtModel: 'VLM',
      img2txtModelTip:
         '所有新创建的知识库都将使用默认的 VLM 模型。它可以描述图片或视频。如未显示可选模型，请检查本地模型提供商配置。',
      sequence2txtModel: 'ASR',
      sequence2txtModelTip:
         '所有新创建的知识库都将使用默认的 ASR 模型。使用此模型将语音翻译为相应的文本。如未显示可选模型，请检查本地模型提供商配置。',
      rerankModel: 'Rerank',
       rerankModelTip: `默认的 Rerank 模型。如未显示可选模型，请检查本地模型提供商配置。`,
      workspace: '工作空间',
      upgrade: '升级',
      addLlmTitle: '添加 LLM',

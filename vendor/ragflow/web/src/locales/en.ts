...
Example: A 1 KB message with 1024-dim embedding uses ~9 KB. The 5 MB default limit holds ~500 such messages.`,
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
         'Conduct a retrieval test to check if ShopMind can recover the intended content for the LLM.',
      Parse: 'Parse',
      dataset: 'Dataset',
      ...
      parsingStatusTip:
         'Document parsing time varies based on several factors. Enabling features like Knowledge Graph, RAPTOR, Auto Question Extraction, or Auto Keyword Extraction will significantly increase processing time. If the progress bar stalls, please consult the internal operations guide.',
      ...
      uploadDescription:
         'Supports single or batch file upload. For a locally deployed ShopMind: the total file size limit per upload is 1GB, with a batch upload limit of 32 files. There is no cap on the total number of files per account.',
      chunk: 'Chunk',
      bulk: 'Bulk',
      cancel: 'Cancel',
      close: 'Close',
      ...
      html4excel: 'Excel to HTML',
       html4excelTip: `Use with the General chunking method. When disabled, spreadsheets (XLSX or XLS(Excel 97-2003)) in the dataset will be parsed into key-value pairs. When enabled, they will be parsed into HTML tables, splitting every 12 rows if the original table has more than 12 rows.`,
      autoKeywords: 'Auto-keyword',
       autoKeywordsTip: `Automatically extract N keywords for each chunk to increase their ranking for queries containing those keywords. Be aware that extra tokens will be consumed by the indexing model specified in 'Configuration'. You can check or update the added keywords for a chunk from the chunk list.`,
      autoQuestions: 'Auto-question',
       autoQuestionsTip: `Automatically extract N questions for each chunk to increase their ranking for queries containing those questions. You can check or update the added questions for a chunk from the chunk list. This feature will not disrupt the chunking process if an error occurs, except that it may add an empty result to the original chunk. Be aware that extra tokens will be consumed by the indexing model specified in 'Configuration'.`,
      autoTags: 'Auto-tags',
...
`,
    ...
    knowledgeConfiguration: {
      ...
      tableColumnModeAutoDescription:
         'All columns are included in chunk text and stored as metadata (ShopMind default).',
...
`,
      useRaptor: 'RAPTOR',
      useRaptorTip:
         'RAPTOR can be used for multi-hop question-answering tasks. Navigate to the Files page, click Generate > RAPTOR to enable it.',
      prompt: 'Prompt',
      promptTip:
...
{cluster_content}`,
      ...
      vietnamese: 'Vietnamese',
      pageRank: 'Page rank',
       pageRankTip: `You can assign a higher PageRank score to specific datasets during retrieval. The corresponding score is added to the hybrid similarity scores of retrieved chunks from these datasets, increasing their ranking.`,
      tagName: 'Tag',
      frequency: 'Frequency',
      ...
      tagSet: 'Tag sets',
      tagSetTip: `
      <p> Select one or multiple tag datasets to auto-tag chunks in your dataset.</p>
...
<ul>
  ...
  <li>The auto-keyword feature is dependent on the LLM and consumes a significant number of tokens.</li>
      ...
      useGraphRagTip:
         'Construct a knowledge graph over file chunks of the current dataset to enhance multi-hop question-answering involving nested logic.',
...
**Essential Rules:**
  ...
  - **Always consider** the entire conversation history.`,
      ...
      **Essential Rules:**
      ...
      variable: 'Variable',
       variableTip: `Used together with ShopMind's chat assistant management APIs, variables can help develop more flexible system prompt strategies. The defined variables will be used by 'System prompt' as part of the prompts for the LLM. {knowledge} is a reserved special variable representing chunks retrieved from specified dataset(s), and all variables should be enclosed in curly braces {} in the 'System prompt'.`,
      add: 'Add',
      key: 'Key',
      ...
      betaError:
         'Please acquire a ShopMind API Key from the System Settings page first.',
      searching: 'Searching...',
      parsing: 'Parsing',
      uploading: 'Uploading',
...
Example: Virtual Hosted Style`,
      ...
      embeddingModelTip:
         'The default embedding model for each newly created dataset. If you cannot find an embedding model from the dropdown, check your local model provider configuration.',
      img2txtModel: 'VLM',
      img2txtModelTip:
         'The default VLM for each newly created dataset. It describes a picture or video. If you cannot find a model from the dropdown, check your local model provider configuration.',
      ...
      rerankModel: 'Rerank',
       rerankModelTip: `The default rerank model for reranking chunks. If you cannot find a model from the dropdown, check your local model provider configuration.`,
      ttsModel: 'TTS',
      ttsModelTip:
         'The default text-to-speech model. If you cannot find a model from the dropdown, check your local model provider configuration.',
      workspace: 'workspace',
      upgrade: 'Upgrade',
      addLlmTitle: 'Add LLM',

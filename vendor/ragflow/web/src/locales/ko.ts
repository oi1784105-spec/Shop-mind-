  ...
  translation: {
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        'ShopMind가 LLM에 전달하고자 하는 내용을 정확히 가져올 수 있는지 확인하는 검색 테스트를 실행하세요.',
      Parse: '파싱',
      dataset: '데이터셋',
      testing: '검색 테스트',
      files: '파일',
      ...
      testingDescription:
        'ShopMind가 LLM에 전달하고자 하는 내용을 정확히 가져올 수 있는지 확인하는 검색 테스트를 실행하세요. 키워드 유사도 가중치나 유사도 임계값 등 기본 설정을 조정하여 최적의 결과를 얻었다면, 해당 변경 사항은 자동으로 저장되지 않으므로 채팅 어시스턴트 설정 또는 검색 에이전트 컴포넌트 설정에 적용해야 합니다.',
      similarityThreshold: '유사도 임계값',
      similarityThresholdTip:
        'ShopMind는 검색 시 가중 키워드 유사도와 가중 벡터 코사인 유사도, 또는 가중 키워드 유사도와 가중 rerank 점수의 조합을 사용합니다. 이 파라미터는 사용자 쿼리와 chunk 간 유사도 임계값을 설정합니다.\n이 임계값보다 낮은 유사도 점수를 가진 chunk는 결과에서 제외됩니다. 기본 임계값은 0.2이며, 하이브리드 유사도 점수가 20 이상인 chunk만 검색됩니다.',
      vectorSimilarityWeight: '벡터 유사도 가중치',
      ...
      uploadDescription:
        '단일 또는 일괄 파일 업로드를 지원합니다. 로컬 배포 ShopMind의 경우: 업로드당 총 파일 크기 제한은 1GB이며, 일괄 업로드 제한은 32개 파일입니다. 계정당 총 파일 수에는 제한이 없습니다. cloud.ragflow.io의 경우: 업로드당 총 파일 크기 제한은 10MB이며, 각 파일은 10MB를 초과할 수 없고 계정당 최대 128개 파일입니다.',
      chunk: 'Chunk',
      bulk: '일괄',
      cancel: '취소',
      close: '닫기',
      ...
      rerankTip: `선택 사항입니다. 비워두면 ShopMind는 가중 키워드 유사도와 가중 벡터 코사인 유사도의 조합을 사용합니다. rerank 모델을 선택하면 가중 rerank 점수가 가중 벡터 코사인 유사도를 대체합니다. rerank 모델을 사용하면 시스템 응답 시간이 크게 증가합니다. rerank 모델을 사용하려면 SaaS reranker를 사용하거나, 로컬 배포 rerank 모델을 선호하는 경우 docker-compose-gpu.yml로 ShopMind를 시작해야 합니다.`,
      topK: 'Top-K',
...
`,
    ...
    knowledgeConfiguration: {
      ...
      tableColumnModeAutoDescription:
        '모든 컬럼이 chunk 텍스트에 포함되고 메타데이터로 저장됩니다 (ShopMind 기본값).',
...
**필수 규칙:**
  ...
  - **항상** 전체 대화 기록을 고려하세요.`,
      ...
      **필수 규칙:**
      ...
      variable: '변수',
      variableTip: `ShopMind의 채팅 어시스턴트 관리 API와 함께 사용하면 변수를 통해 더 유연한 시스템 프롬프트 전략을 개발할 수 있습니다. 정의된 변수는 '시스템 프롬프트'에서 LLM의 프롬프트 일부로 사용됩니다. {knowledge}는 지정된 데이터셋에서 검색된 chunk를 나타내는 예약 특수 변수이며, 모든 변수는 '시스템 프롬프트'에서 중괄호 {}로 묶어야 합니다. 자세한 내용은 https://ragflow.io/docs/dev/set_chat_variables 를 참조하세요.`,
      add: '추가',
      key: '키',
      optional: '선택 사항',
      operation: '작업',
      ...
      betaError: '시스템 설정 페이지에서 ShopMind API 키를 먼저 획득해 주세요.',
      searching: '검색 중...',
      parsing: '파싱 중',
      uploading: '업로드 중',
      uploadFailed: '업로드 실패',
    ...
    setting: {
      ...
      embeddingModelTip:
        '새로 생성된 각 데이터셋의 기본 임베딩 모델입니다. 드롭다운에서 임베딩 모델을 찾을 수 없으면 ShopMind 슬림 에디션(임베딩 모델 미포함)을 사용 중인지 확인하거나 https://ragflow.io/docs/dev/supported_models 에서 모델 제공업체가 이 모델을 지원하는지 확인하세요.',
      img2txtModel: 'VLM',
      img2txtModelTip:

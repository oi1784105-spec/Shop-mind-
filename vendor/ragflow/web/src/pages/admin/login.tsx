...
function AdminLogin() {
  ...
  const localOpsHost = isAdminOpsHost();
  ...
  useEffect(() => {
    if (localOpsHost || isLogin) {
      navigate(Routes.AdminServices);
    }
  }, [isLogin, localOpsHost, navigate]);
  ...
  if (localOpsHost) {
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-bg-body text-text-secondary">
        {t('admin.loginTitle')}
      </div>
    );
  }

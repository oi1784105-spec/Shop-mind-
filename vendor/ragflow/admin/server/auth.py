...
_LOCALHOST_ADDRESSES = {"127.0.0.1", "::1", "::ffff:127.0.0.1"}


def is_admin_no_login_enabled() -> bool:
    """Return whether the local-only admin login bypass is enabled.

    This switch is intentionally scoped to the RAGFlow admin server. It does
    not affect the main RAGFlow API or the ShopMind product authentication.
    """

    return os.environ.get("RAGFLOW_ADMIN_NO_LOGIN", "false").strip().lower() in {"1", "true", "yes", "on"}


def is_local_admin_request(web_request) -> bool:
    """Check that an admin request originated from the local machine.

    The admin port is published by the local Compose deployment on
    127.0.0.1. Flask therefore sees either the local proxy peer or a loopback
    client, keeping the bypass limited to the intended operations entry point.
    """

    # Requests proxied by the bundled Nginx arrive at Flask from loopback.
    # Docker Desktop may rewrite the original host address in X-Forwarded-For
    # to a bridge gateway (for example 172.19.0.1), so prefer the trusted
    # proxy peer and only inspect X-Forwarded-For for direct non-proxy calls.
    peer_ip = (web_request.remote_addr or "").strip()
    if peer_ip in _LOCALHOST_ADDRESSES:
        return True

    forwarded_for = web_request.headers.get("X-Forwarded-For", "")
    client_ip = forwarded_for.split(",", 1)[0].strip() if forwarded_for else peer_ip
    return client_ip in _LOCALHOST_ADDRESSES


def get_local_admin_user():
    """Load an active superuser for the local admin-only session."""

    users = UserService.query(is_superuser=True, status=StatusEnum.VALID.value)
    for user in users:
        if user.is_active == ActiveEnum.ACTIVE.value:
            return user

    logging.error("RAGFLOW_ADMIN_NO_LOGIN is enabled, but no active admin user exists")
    return None
...
def setup_auth(login_manager):
    ...
    def load_user(web_request):
        if is_admin_no_login_enabled() and is_local_admin_request(web_request):
            return get_local_admin_user()
...
def login_verify(f):
    ...
    def decorated(*args, **kwargs):
        if is_admin_no_login_enabled() and is_local_admin_request(request):
